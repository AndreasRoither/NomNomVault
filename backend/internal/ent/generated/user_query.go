...
func (_q *UserQuery) QueryMemberships() *HouseholdMemberQuery {
...
// QueryRecipeShares chains the current query on the "recipe_shares" edge.
func (_q *UserQuery) QueryRecipeShares() *RecipeShareQuery {
...
func (_q *UserQuery) WithMemberships(opts ...func(*HouseholdMemberQuery)) *UserQuery {
	query := (&HouseholdMemberClient{config: _q.config}).Query()
...
// WithRecipeShares tells the query-builder to eager-load the nodes that are connected to
// the "recipe_shares" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithRecipeShares(opts ...func(*RecipeShareQuery)) *UserQuery {
	query := (&RecipeShareClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withMemberships; query != nil {
		if err := _q.loadMemberships(ctx, query, nodes,
			func(n *User) { n.Edges.Memberships = []*HouseholdMember{} },
			func(n *User, e *HouseholdMember) { n.Edges.Memberships = append(n.Edges.Memberships, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withRecipeShares; query != nil {
...
func (_q *UserQuery) loadMemberships(ctx context.Context, query *HouseholdMemberQuery, nodes []*User, init func(*User), assign func(*User, *HouseholdMember)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(householdmember.FieldUserID)
	}
...
func (_q *UserQuery) loadRecipeShares(ctx context.Context, query *RecipeShareQuery, nodes []*User, init func(*User), assign func(*User, *RecipeShare)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(recipeshare.FieldCreatedByUserID)
	}
	query.Where(predicate.RecipeShare(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.RecipeSharesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
