...
func HasMembershipsWith(preds ...predicate.HouseholdMember) predicate.User {
	return predicate.User(func(s *sql.Selector) {
...
// HasRecipeShares applies the HasEdge predicate on the "recipe_shares" edge.
func HasRecipeShares() predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
...
// HasRecipeSharesWith applies the HasEdge predicate on the "recipe_shares" edge with a given conditions (other predicates).
func HasRecipeSharesWith(preds ...predicate.RecipeShare) predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := newRecipeSharesStep()
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
				p(s)
			}
