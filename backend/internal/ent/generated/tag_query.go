...
func (_q *TagQuery) QueryRecipes() *RecipeQuery {
...
func (_q *TagQuery) WithRecipes(opts ...func(*RecipeQuery)) *TagQuery {
	query := (&RecipeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := _q.withRecipes; query != nil {
		if err := _q.loadRecipes(ctx, query, nodes,
			func(n *Tag) { n.Edges.Recipes = []*Recipe{} },
			func(n *Tag, e *Recipe) { n.Edges.Recipes = append(n.Edges.Recipes, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TagQuery) loadRecipes(ctx context.Context, query *RecipeQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Recipe)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.RecipesTable)
		s.Join(joinT).On(s.C(recipe.FieldID), joinT.C(tag.RecipesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(tag.RecipesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.RecipesPrimaryKey[1]))
	...
	neighbors, err := withInterceptors[[]*Recipe](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "recipes" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (_q *TagQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withHousehold != nil {
			_spec.Node.AddColumnOnce(tag.FieldHouseholdID)
		}
	}
